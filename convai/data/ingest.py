...
class MovieLensLoader:
            ...
            with open(genres_file, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    if len(row) >= 2:
                        ...
                        genre = Genre(genre_id=genre_id, name=genre_name)
                        db.add(genre)
                        count += 1
                db.commit()
            ...
            with open(users_file, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    if len(row) >= 5:
                        ...
                        existing = (
                            db.query(User).filter(User.user_id == int(user_id)).first()
                        )
                        if not existing:
                            user = User(
                                user_id=int(user_id),
            ...
            with open(items_file, "r", encoding="latin-1") as f:
                ...
                for row in reader:
                    if len(row) >= 24:
                        ...
                        existing = (
                            db.query(Movie).filter(Movie.movie_id == movie_id).first()
                        )
                        if not existing:
                            movie = Movie(
                                movie_id=movie_id,
    ...
    @staticmethod
    def load_all(
        db: Session, data_path: str = "./ml-100k"
