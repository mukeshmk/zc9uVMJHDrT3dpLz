...
class Agent:
    def __init__(self, llm: BaseChatModel):
        ...
        def custom_prompt(request: ModelRequest) -> str:
            ...
            conversation_history = request.runtime.context.get(
        ...
        self.agent = create_agent(
            model=self.llm, tools=toolkit.get_tools(), middleware=[custom_prompt]
        )
    ...
    def generate_and_execute(self, state: GraphState) -> GraphState:
        ...
        try:
            ...
            logger.debug(
                f"SQL generation context - Intent: {state['intent'].intent.value}, Entities: {state['entities']}"
            )
            ...
            for step in self.agent.stream(
                ...
                step_count += 1
                logger.debug(
                    f"Tool Calling Agent step {step_count}: {json.dumps(step, indent=2, default=str)}"
                )
            ...
            logger.info(
                f"Tool Calling Agent completed successfully after {step_count} steps"
            )
