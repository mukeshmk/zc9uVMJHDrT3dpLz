...
class SmartRouter:
    def __init__(self, llm: BaseChatModel):
        ...
        self.chain = self.prompt | self.llm.with_structured_output(RouterDecision)
    ...
    def route_query(self, state: GraphState) -> GraphState:
        ...
        try:
            ...
            logger.info(
                f"Routing decision - Route: {result.route}, Confidence: {result.confidence:.2f}, Reason: {result.reason}"
            )
            state["route"] = result.route
            if result.clarification_message:
                logger.debug(
                    f"Clarification message provided: {result.clarification_message}"
                )
