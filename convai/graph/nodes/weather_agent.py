        ...
        try:
            if self.mcp_server.startswith("http"):
                ...
                params = StdioServerParameters(
                    command=command, args=[self.mcp_server], env=None
                )
                ...
                raise ValueError(
                    "Must provide either url or server_script_path for MCP_SERVER environment variable."
                )

            await self.session.initialize()

            tools = await load_mcp_tools(session=self.session)
